...
def upload_file_to_latest_archive(data_file, gzip_file):
    ...
    ## to latest:
    s3_file = "/latest/" + data_file.replace("scripts/dumping/ncbi/data/", "")
    s3.meta.client.upload_file(data_file, S3_BUCKET,  s3_file, ExtraArgs={'ACL': 'public-read'})

    ## to current directoty under sgd-archive.yeastgenome.org bucket
    s3_file = s3_archive_dir + data_file.replace("scripts/dumping/ncbi/data/", "")
    s3.meta.client.upload_file(data_file, S3_BUCKET2, s3_file, ExtraArgs={'ACL': 'public-read'})

    ## to archive directory under sgd-archive.yeastgenome.org bucket
    s3_file = s3_archive_dir + "archive/" + gzip_file.replace("scripts/dumping/ncbi/data/", "")
    s3.meta.client.upload_file(gzip_file, S3_BUCKET2, s3_file, ExtraArgs={'ACL': 'public-read'})
