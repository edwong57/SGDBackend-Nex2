...
BEGIN
  IF (TG_OP = 'UPDATE') THEN
    ...
    IF  (((OLD.curator_comment IS NULL) AND (NEW.curator_comment IS NOT NULL)) OR ((OLD.curator_comment IS NOT NULL) AND (NEW.curator_comment IS NULL)) OR (OLD.curator_comment != NEW.curator_comment)) THEN
        PERFORM nex.insertupdatelog('FUNCTIONALCOMPLEMENTANNOTATION'::text, 'DISEASE_QUALIFIER'::text, OLD.annotation_id, OLD.disease_qualifier::text, NEW.disease_qualifier::text, USER);
    END IF;

    IF (OLD.date_assigned != NEW.date_assigned) THEN
        PERFORM nex.insertupdatelog('FUNCTIONALCOMPLEMENTANNOTATION'::text, 'DATE_ASSIGNED'::text, OLD.annotation_id, OLD.date_assigned::text, NEW.date_assigned::text, USER);
    END IF;
  ...
  ELSIF (TG_OP = 'DELETE') THEN
    ...
    v_row := OLD.annotation_id || '[:]' || OLD.dbentity_id || '[:]' ||
             ...
             OLD.reference_id || '[:]' || OLD.eco_id || '[:]' || OLD.ro_id || '[:]' ||
             coalesce(OLD.curator_comment,'') || '[:]' || OLD.date_assigned || '[:]' ||
